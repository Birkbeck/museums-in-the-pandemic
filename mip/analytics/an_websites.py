...
import re
from utils import remove_empty_elem_from_list
import logging
...
def insert_page_attribute(db_con, page_id, session_id, attrib_name, attrib_val):
    assert attrib_name in ['title','headers']
    assert page_id >= 0
    if attrib_val == '':
        attrib_val = None
...
def extract_attributes_from_page_html(db_con, page_id, session_id, page_html):
    ...
    insert_page_attribute(db_con, page_id, session_id, 'headers', field_sep.join(headers))

    # TODO: extract other fields from HTML
