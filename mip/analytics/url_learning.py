...
def assign_urls_to_sample():
    stratdf = pd.read_csv(
        'data/museums/museums_wattributes-2020-02-23.tsv', sep='\t')
    googledf = pd.read_csv(
        'data/google_results/results_source_files/google_extracted_results_reg.tsv.gz', sep='\t')
    facebookdf = pd.read_csv(
        'data/google_results/results_source_files/google_extracted_results_facebook.tsv.gz', sep='\t')
    twitterdf = pd.read_csv(
        'data/google_results/results_source_files/google_extracted_results_twitter.tsv.gz', sep='\t')
    sampledf = load_museum_samples()
    #sampledf.drop(['search'], axis=1)

    museweight = generate_weighted_museum_names()
    stratdf = stratdf.filter(
        ['muse_id', 'musname', 'size', 'governance', 'town'], axis=1)
    googledf = googledf.filter(
        ['muse_id', 'search_type', 'url', 'google_rank'], axis=1)
    facebookdf = facebookdf.filter(
        ['muse_id', 'search_type', 'url', 'google_rank'], axis=1)
    twitterdf = twitterdf.filter(
        ['muse_id', 'search_type', 'url', 'google_rank'], axis=1)
    ...
    outputdf['has_visit'] = outputdf.apply(lambda row: (
        hasvisit(row['url'], row['search_type'])), axis=1)
    outputdf['has_museum'] = outputdf.apply(lambda row: (
        hasmuseum(row['url'], row['search_type'])), axis=1)
    outputdf['has_location'] = outputdf.apply(
        lambda row: (haslocation(row['url'], row['town'])), axis=1)

    outputdf['fuzzy_score_full_url'] = outputdf.apply(lambda row: (generate_weighted_fuzzy_scores(
        get_musname_pool(row['musname'], row['town']), row['url'], museweight, "", False, row['search_type'])), axis=1)
    outputdf['fuzzy_score_domain'] = outputdf.apply(lambda row: (generate_weighted_fuzzy_scores(
        get_musname_pool(row['musname'], row['town']), row['url'], museweight, "", True, row['search_type'])), axis=1)
    outputdf['fuzzy_score_domain_inverse'] = outputdf.apply(lambda row: (get_fuzzy_string_score(
        get_url_domain_with_search(row['url'], row['search_type']), row['musname'])), axis=1)
    outputdf['withloc_fuzzy_score_full_url'] = outputdf.apply(lambda row: (generate_weighted_fuzzy_scores(
        get_musname_pool(row['musname'], row['town']), row['url'], museweight, row['town'], False, row['search_type'])), axis=1)
    outputdf['withloc_fuzzy_score_domain'] = outputdf.apply(lambda row: (generate_weighted_fuzzy_scores(
        get_musname_pool(row['musname'], row['town']), row['url'], museweight, row['town'], True, row['search_type'])), axis=1)

    outputdf['exact_score_url'] = outputdf.apply(lambda row: (
        get_exact_match(row['musname'], row['url'])), axis=1)
    outputdf['exact_score_url_inverse'] = outputdf.apply(
        lambda row: (get_exact_match(row['url'], row['musname'])), axis=1)
    outputdf['exact_score_domain'] = outputdf.apply(lambda row: (get_exact_match(
        row['musname'], get_url_domain_with_search(row['url'], row['search_type']))), axis=1)
    outputdf['exact_score_domain_inverse'] = outputdf.apply(lambda row: (get_exact_match(
        get_url_domain_with_search(row['url'], row['search_type']), row['musname'])), axis=1)
    print(len(outputdf))
    outputdf = pd.merge(outputdf, sampledf, on=[
                        'muse_id', 'search_type'], how='left')
    print(len(outputdf))
    # TO DO:
    ...
    outputdf = outputdf.sort_values(
        ['muse_id', 'search_type', 'google_rank'], ascending=[True, True, True])
...
def get_best_forest(df_ml_matrix, name):
    df_ml_matrix_out = df_ml_matrix.loc[df_ml_matrix['iscorrect'] != '', [
        'muse_id',	'url', 'musname']]
    ...
    split = 0.2
    res_df = pd.DataFrame(
        columns=['precision', 'accuracy', 'sensetivity', 'specificity'])
    ...
    for z in range(0, 300):
        ...
        accuracy = (confusmat[0][0]+confusmat[1][1])/(confusmat[0]
                                                      [0]+confusmat[0][1]+confusmat[1][0]+confusmat[1][1])
...
def test_ml(self):
    ...
    df_matrix2 = pd.read_csv(
        'data/samples/mip_data_sample_2020_01.tsv', sep='\t')
    df_matrix2 = df_matrix2[['mm_id', 'website']]
    df_matrix3 = df_matrix.merge(
        df_matrix2, left_on='id', right_on='mm_id', how='left')
...
def generate_ml_model():
    ...
    df_matrix['valid'] = df_matrix.apply(
        lambda row: (repair_valid_column(row['valid'])), axis=1)
