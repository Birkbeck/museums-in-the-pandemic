...
def get_museums_w_web_urls(data_folder=''):
    ...
    msg = "get_museums_w_web_urls Museums={} URLs={}".format(
        df.muse_id.nunique(), len(df))
    print(msg)
    logger.info(msg)
    return df
...
def combine_museums_w_web_urls():
    ...
    mandf = mandf[['muse_id', 'musname',
                   'town', 'url', 'search_type', 'valid']]
...
def get_fb_tw_links():
    ...
    df = pd.read_csv(
        'data/google_results/museum_searches_all-2021-02-16.tsv', sep='\t')
    ...
    for muse_id, muse_df in df.groupby('muse_id'):
        ...
        row = pd.DataFrame({'muse_id': muse_id, 'links_n': len(muse_df), 'twitter_link_n': len(
            tw_df), 'facebook_link_n': len(fb_df)}, index=[muse_id])
        stats_df = stats_df.append(row)
        # save fb/tw links
        row = pd.DataFrame({'muse_id': muse_id, 'muse_name': muse_name,
                           'top_facebook': None, 'top_twitter': None}, index=[muse_id])
        if len(tw_urls) > 0:
            row['top_twitter'] = tw_urls[0]
        if len(fb_urls) > 0:
            row['top_facebook'] = fb_urls[0]
...
def load_extracted_museums(df):
    ...
    dfcheck = pd.DataFrame(
        columns=["google_rank", "url", "search", "muse_id", "location", "Museum_Name"])
    ...
    for item in df.iterrows():
        ...
        if item[1].google_rank == 1:
            ...
            if comparedf['website'].str.contains(urlstring).any():
                ...
                list1 = [item[1].google_rank, item[1].url, item[1].search,
                         item[1].muse_id, item[1].location, item[1].Museum_Name]
                dftoadd = pd.DataFrame([list1], columns=[
                                       "google_rank", "url", "search", "muse_id", "location", "Museum_Name"])
...
def generate_samples():
    resultsdf = pd.read_csv(
        "data/google_results/results_source_files/fuzzy_museum_scores_twitter.tsv", sep='\t')
        ...
        if row[1].search == search:
            ...
            for item in resultsdf.iterrows():
                ...
                if item[1].id == row[1].muse_id:
                    rowfound = True
                    targetrow = pd.DataFrame({'muse_id': item[1].id, 'muse_name': item[1].Museum_Name, 'url': item[1].url,
                                             'coorect_url': row[1].correct_url, 'google_rank': item[1].google_rank, 'score': item[1].score}, index=[item[1].id])
                ...
                if item[1].id == row[1].muse_id:
                    ...
                    if not correcturl == 'no_resource':
                        if get_url_domain_with_search(itemurl, search) == get_url_domain_with_search(correcturl, search):
                            print("yes "+get_url_domain_with_search(itemurl, search) +
                                  " "+get_url_domain_with_search(correcturl, search))
                            score = score+1
                        else:
                            print("no "+get_url_domain_with_search(itemurl, search) +
                                  " "+get_url_domain_with_search(correcturl, search))
...
def load_manual_museum_urls():
    ...
    manual_sites_df = df[df.correct_site.notnull()][[
        'muse_id', 'correct_site']]
    manual_sites_df = manual_sites_df[manual_sites_df.correct_site.str.contains(
        'http')]
...
def get_extra_museum_attributes(df):
    ...
    df['region'] = np.where(df['country'] == 'England',
                            df['region'], df['country'])
    df['region'] = df['region'].str.replace(
        '\(English Region\)', '').str.strip()
...
def generate_stratified_museum_sample():
    ...
    print(df2.columns)
    print("difference between datasets:", set(
        df1.id).symmetric_difference(set(df2.muse_id)))
        ...
        if location != "":
            if 'loc' in key:
                if 'abbrev' in key:
                    ...
                    score = get_fuzzy_musname_score(
                        mname[key], str_from_url, weighteddict, weightsum)
                    scores.append(score)
                    # print(key)
                    # print(score)
...
def generate_combined_dataframe():
    ...
    df_mus_exact = pd.read_csv(
        'data/google_results/results_source_files/google_extracted_results_exact.tsv.gz', sep='\t')
    ...
    scrapetarget = []
    searchtype = []
    df_facebook_noloc = pd.read_csv(
        'data/google_results/results_source_files/google_extracted_results_facebook_noloc.tsv.gz', sep='\t')
    ...
    scrapetarget = []
    searchtype = []
    df_twitter_noloc = pd.read_csv(
        'data/google_results/results_source_files/google_extracted_results_twitter_noloc.tsv.gz', sep='\t')
    ...
    finaldf.to_csv(
        'data/google_results/google_results_all_02_03_2021.tsv', index=False, sep='\t')
...
def get_fuzzy_string_match_scores(musdf, search):
    ...
    for row in musdf.iterrows():
        ...
        if not isinstance(location, float) and not isinstance(location, int):
            ...
            location = ""
        if urlstring != '':
            scorerow.append(generate_weighted_fuzzy_scores(
                musename, urlstring, museweight, location))
        else:
            scorerow.append(0)
    musdf['score'] = scorerow
    finaldf = pd.DataFrame()

    newdf = musdf.sort_values(
        ['muse_id', 'score', 'google_rank'], ascending=[True, False, True])
...
def generate_weighted_museum_names():
    df1 = pd.read_csv(
        'data/museums/museum_names_and_postcodes-2020-01-26.tsv', sep='\t')
    joiningwords = ["or", "the", "a", "for", "th", ""]
    weighteddict = {}
...
def load_all_google_results():
    df = pd.read_csv(
        'data/google_results/google_results_all_02_03_2021.tsv.gz', sep='\t')
...
def join_museum_info(df, muse_id_column):
    ...
    mdf = df.merge(museums_df, left_on=muse_id_column,
